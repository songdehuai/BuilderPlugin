...
public class ExportChannelToFile extends DefaultTask {

    public ExportChannelToFile() {
    ...
    List<Flavor> getFlavors(AppExtension android, String projectFile) {
        ...
        for (ProductFlavor flavor : flavors) {
            ...
            try {
                File fileTemp = new File(outPutFilePath + mFlavor.getName() + ".json");
    ...
    public void mvFlavor(String projectFile, String outPutFilePath, String flavorName) {
        File mipmap_xxhdpi = new File(projectFile + "/app/src/" + flavorName + "/res/mipmap-xxhdpi/icon_launcher.png");
        File drawable_xxhdpi = new File(projectFile + "/app/src/" + flavorName + "/res/drawable-xxhdpi/loading.png");
        try {
            FileUtils.copyFileToDirectory(mipmap_xxhdpi, new File(outPutFilePath));
            FileUtils.copyFileToDirectory(drawable_xxhdpi, new File(outPutFilePath));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
